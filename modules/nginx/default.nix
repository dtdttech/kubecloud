...
let
  ...
  generateNginxConfig = {
    ...
    upstreamBlocks = lib.concatStringsSep "\n" (lib.mapAttrsToList (upstreamName: upstream: ''
      upstream ${upstreamName} {
        ...
        ${lib.optionalString (upstream.keepalive or null != null) "keepalive ${toString upstream.keepalive};"}
      }
    '') upstreams);
    ...
    serverBlocks = lib.concatStringsSep "\n" (lib.mapAttrsToList (siteName: site: ''
      server {
        listen ${toString (site.port or 80)}${lib.optionalString (site.defaultServer or false) " default_server"};
  ...
  createNginxDeployment = name: nginxConfig: let
    ...
    volumes = lib.optionalAttrs (nginxConfig.storage.config.enable or false) {
            ...
            template = {
              ...
              spec = {
                ...
                volumes = [
                  ...
                  {
                    name = "tls";
                    secret.secretName = if nginxConfig.tls.useExisting then nginxConfig.tls.existingSecret else "${name}-tls";
                  }
...
in {
  ...
  config = {
    ...
    inherit (lib.foldl' (acc: name: 
      let nginxConfig = cfg.deployments.${name};
      in if nginxConfig.enable 
         then acc // (createNginxDeployment name nginxConfig)
         else acc
    ) {} (lib.attrNames cfg.deployments));

    # Add monitoring rules if any nginx deployment has monitoring enabled
    monitoring.prometheus.rules = lib.mkIf (lib.any (d: d.monitoring.enabled or false) (lib.attrValues cfg.deployments)) [
      {
        name = "nginx";
        rules = [
          {
