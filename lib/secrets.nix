...
rec {
  ...
  createSecret = {
    ...
    if provider == "internal" then
      createInternalSecret {
        inherit name namespace type data stringData labels annotations;
      }
    else
      createExternalSecret {
        inherit name namespace type secretStore secretStoreKind refreshInterval labels annotations template;
        data = externalData;
      };
  ...
  commonSecrets = {
    ...
    database = { name, provider ? "internal", username, password, host ? null, port ? null, database ? null }:
      let
        secretData = {
          username = username;
          password = password;
      ...
      createSecret {
        ...
        externalData = if provider != "internal" then [
          { secretKey = "username"; key = "${name}-username"; }
          { secretKey = "password"; key = "${name}-password"; }
        ] ++ lib.optionals (host != null) [
          { secretKey = "host"; key = "${name}-host"; }
        ] ++ lib.optionals (port != null) [
          { secretKey = "port"; key = "${name}-port"; }
        ] ++ lib.optionals (database != null) [
          { secretKey = "database"; key = "${name}-database"; }
        ] else [];
      };

    # API keys and tokens
    apiKey = { name, provider ? "internal", key, description ? null }:
      createSecret {
        inherit name provider;
        type = "generic";
        stringData = if provider == "internal" then { api-key = key; } else {};
        externalData = if provider != "internal" then [
          { secretKey = "api-key"; key = "${name}-api-key"; }
        ] else [];
    ...
    # TLS certificates
    tls = { name, provider ? "internal", cert ? null, key ? null, ca ? null }:
      createSecret {
        inherit name provider;
        type = "tls";
        stringData = if provider == "internal" then {
          "tls.crt" = cert;
          "tls.key" = key;
        } // lib.optionalAttrs (ca != null) { "ca.crt" = ca; } else {};
        externalData = if provider != "internal" then [
          { secretKey = "tls.crt"; key = "${name}-tls-cert"; }
          { secretKey = "tls.key"; key = "${name}-tls-key"; }
        ] ++ lib.optionals (ca != null) [
          { secretKey = "ca.crt"; key = "${name}-ca-cert"; }
        ] else [];
      };

    # Docker registry credentials
    dockerRegistry = { name, provider ? "internal", server, username, password, email ? null }:
      let
        dockerConfig = {
          auths.${server} = {
      ...
      createSecret {
        ...
        externalData = if provider != "internal" then [
          { secretKey = ".dockerconfigjson"; key = "${name}-dockerconfig"; }
        ] else [];
      };

    # Basic authentication
    basicAuth = { name, provider ? "internal", username, password }:
      createSecret {
        inherit name provider;
        type = "basic-auth";
        stringData = if provider == "internal" then {
    ...
    # SSH keys
    sshKey = { name, provider ? "internal", privateKey, publicKey ? null, knownHosts ? null }:
      createSecret {
        inherit name provider;
        type = "ssh-auth";
        stringData = if provider == "internal" then {
          ...
          // lib.optionalAttrs (knownHosts != null) { "known_hosts" = knownHosts; } else {};
        externalData = if provider != "internal" then [
          { secretKey = "ssh-privatekey"; key = "${name}-ssh-private-key"; }
        ] ++ lib.optionals (publicKey != null) [
          { secretKey = "ssh-publickey"; key = "${name}-ssh-public-key"; }
        ] ++ lib.optionals (knownHosts != null) [
          { secretKey = "known_hosts"; key = "${name}-known-hosts"; }
        ] else [];
      };

    # Generic application secrets
    application = { name, provider ? "internal", secrets }:
      createSecret {
        inherit name provider;
        type = "generic";
  ...
  createSecrets = secretSpecs: { provider ? "internal", secretStore ? null }:
    lib.listToAttrs (map (spec: {
      name = "${spec.name}-secret";
      value = createSecret (spec // { 
        inherit provider;
